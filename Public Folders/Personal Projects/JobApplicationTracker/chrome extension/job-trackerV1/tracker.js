...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // === Charts Modal Logic ===
  let statusPieChart, locationBarChart, dateLineChart, salaryBarChart;
  ...
  function showChartsModal(applications) {
    ...
    applications.forEach((app, idx) => {
      ...
      // Date
      if (app.appliedDate) {
        dateCounts[app.appliedDate] = (dateCounts[app.appliedDate] || 0) + 1;
      }
      // Salary
      const avgSalary = parseSalary(app.salary);
      if (avgSalary) {
        salaryLabels.push(app.company + ' (' + (app.position || 'N/A') + ')');
    ...
    const dateData = sortedDates.map(d => dateCounts[d]);
    if (dateLineChart) dateLineChart.destroy();
    dateLineChart = new Chart(document.getElementById('dateLineChart').getContext('2d'), {
      type: 'line',
      data: {
        labels: sortedDates,
        datasets: [{
    ...
    // Bar chart for salaries with average line
    if (salaryBarChart) salaryBarChart.destroy();
    const avg = salaryData.length ? (salaryData.reduce((a, b) => a + b, 0) / salaryData.length) : 0;
    salaryBarChart = new Chart(document.getElementById('salaryBarChart').getContext('2d'), {
      type: 'bar',
      data: {
        labels: salaryLabels,
        datasets: [
