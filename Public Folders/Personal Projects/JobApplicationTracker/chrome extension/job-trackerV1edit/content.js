...
;(() => {
  console.log("Job Application Tracker content script loaded on:", window.location.hostname);
  ...
  function extractDescription() {
    ...
    const siteSpecificSelectors = {
      "linkedin.com": [
        "#main > div > div.scaffold-layout__list-detail-inner.scaffold-layout__list-detail-inner--grow > div.scaffold-layout__detail.overflow-x-hidden.jobs-search__job-details > div > div.jobs-search__job-details--container > div > div.job-view-layout.jobs-details > div:nth-child(1) > div > div:nth-child(4) > div.job-details-about-the-job-module__description > div",
        ".description__text",
  ...
  function setupApplyDetection() {
    ...
    document.addEventListener("click", (e) => {
      console.log("Click detected on element:", e.target.outerHTML);
      ...
      if (!isApplyButton && (e.target.tagName === 'BUTTON' || e.target.tagName === 'A') && e.target.textContent && e.target.textContent.toLowerCase().includes('apply')) {
        isApplyButton = true;
        console.log("Fallback: Detected apply button or link by text content.");
      }
